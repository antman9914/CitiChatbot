...
from sogou_mrc.nn.layers import Conv1DAndMaxPooling, Dropout, VariationalDropout, Highway, Embedding, ElmoEmbedding, \
    SumMaxEncoder
...
class BiDAFPlus(BaseModel):
    def __init__(self, vocab, pretrained_word_embedding=None, word_embedding_size=100, char_embedding_size=20,
                 char_conv_filters=100,
                 char_conv_kernel_size=5, rnn_hidden_size=100, document_size=3,
                 dropout_keep_prob=0.8, max_answer_len=11, word_embedding_trainable=False, ema_decay=0., use_elmo=False,
                 elmo_local_path=None, abstractive_answer=[], max_pooling_mask=False):
    ...
    def _build_graph(self):
        self.context_word = tf.placeholder(tf.int32, [self.document_size, None, None])
        self.context_len = tf.placeholder(tf.int32, [self.document_size, None])

        self.context_char = tf.placeholder(tf.int32, [self.document_size, None, None, None])
        self.context_word_len = tf.placeholder(tf.int32, [self.document_size, None, None])
        ...
        self.answer_start = tf.placeholder(tf.int32, [self.document_size, None])
        self.answer_end = tf.placeholder(tf.int32, [self.document_size, None])
        self.abstractive_answer_mask = tf.placeholder(tf.int32, [self.document_size, None, self.abstractive_answer_num])
        self.training = tf.placeholder(tf.bool, [])

        self.question_tokens = tf.placeholder(tf.string, [None, None])
        self.context_tokens = tf.placeholder(tf.string, [self.document_size, None, None])
        ...
        char_embedding = Embedding(embedding_shape=(len(self.vocab.get_char_vocab()) + 1, self.char_embedding_size),
                                   trainable=True, init_scale=0.05)
        ...
        modeled_context2, _ = context_modeling_gru2(tf.concat([context_repr, modeled_context1], axis=2),
                                                    self.context_len)
        ...
        modeled_question2, _ = question_modeling_gru2(tf.concat([question_repr, modeled_question1], axis=2),
                                                      self.question_len)
        ...
        if self.abstractive_answer_num != 0:
            ...
            for i in range(self.abstractive_answer_num):
                ...
                abstractive_answer_logits.append(
                    tf.squeeze(tri_linear(encoded_context, encoded_question), squeeze_dims=[2]))
        ...
        start_log_score = tf.reduce_logsumexp(
            concated_start_logits + VERY_NEGATIVE_NUMBER * (1 - tf.cast(concated_start_masks, tf.float32)), axis=1)
        ...
        end_log_score = tf.reduce_logsumexp(
            concated_end_logits + VERY_NEGATIVE_NUMBER * (1 - tf.cast(concated_end_masks, tf.float32)), axis=1)
    ...
    def get_best_answer(self, output, instances):
        ...
        for i in range(len(instances)):
            ...
            if "qid" in instance:
                qid = instance["qid"]
                if isinstance(qid, list):  # coqa qid
                    qid = tuple(qid)
                answer_dict[qid] = pred_answer
            else:
                answer_list.append(pred_answer)  # squad1
        if answer_list:
            return answer_list
        return answer_dict
