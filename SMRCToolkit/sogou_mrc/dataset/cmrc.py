...
class CMRCReader(BaseReader):
    ...
    def _read(self, file_path):
        ...
        for article in dataset:  # article是一篇文章加上问题、答案
            context = article["context_text"].replace(' ','')  # context是文章的内容
            context = Space.remove_white_space(context)  # 去掉空格
            context_tokens, context_token_spans = self.tokenizer.word_tokenizer(context)
            print("context_tokens :",context_tokens)
            print("context_token_spans :",context_token_spans)
            ...
            for question_answer in article['qas']:
                ...
                if "answers" in question_answer:
                    ...
                    print("answers in question_answer:",answers)
                    try:
