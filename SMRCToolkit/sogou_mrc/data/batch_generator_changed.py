...
class BatchGenerator(object):
    ...
    def detect_input_type_shape(instance, additional_fields=None):
        ...
        for field in fields:
            if instance[field] is None:
                print(str(instance[field])+" is none")
    ...
    def build_input_pipeline(self):
        ...
        input_fields, input_type_dict, input_shape_dict = \
            BatchGenerator.detect_input_type_shape(self.instances, self.additional_fields)

        # 1. Get data
        def make_generator():
            #for instance in self.instances:
            new_dict = {k: self.instances[k] for k in input_fields}
            print(456789)
            return new_dict
